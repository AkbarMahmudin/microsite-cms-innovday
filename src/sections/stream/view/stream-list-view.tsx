...
export default function StreamListView() {
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      ...
      >
        {['all', 'published', 'scheduled', 'draft', 'private', 'archived', 'unpublished'].map(
          (tab) => (
            <Tab
              key={tab}
              iconPosition="end"
              ...
              icon={
                ...
                >
                  ...
                  {tab === 'scheduled' &&
                    posts.filter((post) => post.publish === 'scheduled').length}

                  {tab === 'draft' && posts.filter((post) => post.publish === 'draft').length}

                  {tab === 'private' && posts.filter((post) => post.publish === 'private').length}

                  {tab === 'archived' && posts.filter((post) => post.publish === 'archived').length}

                  {tab === 'unpublished' &&
                    posts.filter((post) => post.publish === 'unpublished').length}
                </Label>
              }
      ...
      <StreamListHorizontal posts={dataFiltered} loading={postsLoading} />
    </Container>
  );
}
